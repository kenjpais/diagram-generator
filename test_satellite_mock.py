...
satellite_intent = GraphIntent(
    ...
    groups=[
        ...
        Group(
            id="satellite_env", label="Satellite Server Environment", type="datacenter"
        ),
    ...
    components=[
        ...
        Component(
            ...
            type="server",
            parent_group="satellite_env",
        ),
        # Capsule Servers
        Component(
            id="capsule_server_1",
            label="Capsule Server 1",
            type="server",
            parent_group="capsule_env",
        ),
        Component(
            id="capsule_server_2",
            label="Capsule Server 2",
            type="server",
            parent_group="capsule_env",
        ),
        # Services on Capsule
        ...
        # Managed Hosts
        Component(
            id="host_1", label="Managed Host 1", type="host", parent_group="host_env"
        ),
        Component(
            id="host_2", label="Managed Host 2", type="host", parent_group="host_env"
        ),
        Component(
            id="capsule_base_host",
            label="Capsule Base Host",
            type="host",
            parent_group="capsule_env",
        ),
    ],
    relationships=[
        ...
        Relationship(
            source="redhat_portal",
            target="satellite_server",
            label="synchronizes content",
            type="data_flow",
        ),
        Relationship(
            source="other_sources",
            target="satellite_server",
            label="synchronizes content",
            type="data_flow",
        ),
        ...
        Relationship(
            source="satellite_server",
            target="capsule_server_1",
            label="manages and mirrors content",
            type="data_flow",
        ),
        Relationship(
            source="satellite_server",
            target="capsule_server_2",
            label="manages and mirrors content",
            type="data_flow",
        ),
        # Capsule services
        Relationship(
            source="capsule_server_1",
            target="puppet_master",
            label="provides",
            type="dependency",
        ),
        Relationship(
        ...
        Relationship(
            source="capsule_server_1",
            target="host_1",
            label="provides content and config",
            type="api_call",
        ),
        Relationship(
            source="capsule_server_1",
...
def test_graph_generation():
    ...
    print(colorize("Testing Red Hat Satellite Architecture Diagram Generation", Colors.CYAN, bold=True))
    print(colorize("=" * 60, Colors.CYAN))
    ...
    print_info("Step 1: Generating DOT code...")
    try:
        dot_code = generate_diagram_code(satellite_intent)
        print_success(f"Generated DOT code ({len(dot_code)} characters)")
        print("\nFirst 500 characters of DOT code:")
        print(dot_code[:500])
        print("...\n")
    except Exception as e:
        print_error(f"Failed to generate DOT code: {e}")
        import traceback
    ...
    print_info("Step 2: Validating syntax...")
    try:
